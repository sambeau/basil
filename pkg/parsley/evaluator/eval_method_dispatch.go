...
func dispatchMethodCall(left Object, method string, args []Object, env *Environment) Object {
	...
	case *Unit:
		return evalUnitMethod(receiver, method, args)
	case *Record:
		return evalRecordMethod(receiver, method, args, env)
	case *DSLSchema:
		return evalDSLSchemaMethod(receiver, method, args, env)
	case *Dictionary:
		...
		if isDurationDict(receiver) {
			return evalDurationMethod(receiver, method, args, env)
		}
		if isPathDict(receiver) {
			...
			if errObj, ok := result.(*Error); ok && errObj.Code == "UNDEF-0002" {
				return evalDictionaryMethod(receiver, method, args, env)
			}
			return result
		}
