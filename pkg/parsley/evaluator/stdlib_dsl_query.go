...
func evalQueryExpression(node *ast.QueryExpression, env *Environment) Object {
	...
	// Apply row transform if present
	if node.RowTransform != nil && !isError(result) && result != NULL {
		result = applyRowTransform(result, node.RowTransform, env)
	}

	return result
}

// applyRowTransform applies a row transformation to query results
func applyRowTransform(result Object, transform *ast.RowTransform, env *Environment) Object {
	switch r := result.(type) {
	case *Array:
		// Transform each row in the array
		transformed := make([]Object, 0, len(r.Elements))
		for i, row := range r.Elements {
			transformedRow := transformSingleRow(row, transform, env, i+1)
			if isError(transformedRow) {
				return transformedRow
			}
			transformed = append(transformed, transformedRow)
		}
		return &Array{Elements: transformed}

	case *Dictionary:
		// Transform single row result
		return transformSingleRow(r, transform, env, 1)

	default:
		// For other types (like Integer for count), don't transform
		return result
	}
}

// transformSingleRow applies the row transform to a single row
func transformSingleRow(row Object, transform *ast.RowTransform, env *Environment, rowNum int) Object {
	// Create a new scope for the transform
	transformEnv := NewEnclosedEnvironment(env)

	// Bind the row to the transform binding
	binding := transform.Binding
	if binding.Identifier != nil {
		// Simple binding: as row
		transformEnv.Set(binding.Identifier.Value, row)
	} else if binding.Destructure != nil {
		// Destructure binding: as {a, b, ...rest}
		result := evalDictDestructuringAssignment(binding.Destructure, row, transformEnv, true, false)
		if isError(result) {
			return &Error{
				Message: fmt.Sprintf("row %d: destructuring failed: %s", rowNum, result.(*Error).Message),
	...
	// Evaluate the transform body
	result := Eval(transform.Body, transformEnv)
	if isError(result) {
		// Add row context to error
		err := result.(*Error)
		return &Error{
			Message: fmt.Sprintf("row %d: %s", rowNum, err.Message),
			Class:   err.Class,
			Code:    err.Code,
		}
	}
