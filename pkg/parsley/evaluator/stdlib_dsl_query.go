...
func buildSelectSQL(node *ast.QueryExpression, binding *TableBinding, env *Environment) (string, []Object, *Error) {
	...
	for _, col := range selectCols {
		...
		// Skip aggregate expressions like "COUNT(*) as order_count", "SUM(amount) as total"
		// These are generated by the query DSL and are safe (not user-controlled identifiers)
		if strings.Contains(col, "(") && strings.Contains(col, ") as ") {
			// Extract and validate just the alias after "as"
			parts := strings.Split(col, " as ")
			if len(parts) == 2 {
				alias := parts[1]
				if err := validateSQLIdentifier(alias); err != nil {
					return "", nil, &Error{
						Message: fmt.Sprintf("invalid column alias in projection: %s", alias),
						Class:   ClassValue,
						Code:    "VAL-0003",
					}
				}
				continue // Skip full validation for aggregate expressions
			}
		}
