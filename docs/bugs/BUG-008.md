---
id: BUG-008
title: "Absolute path components[0] is always empty string"
status: resolved
priority: medium
reported: 2025-12-02
resolved: 2025-12-02
...
# BUG-008: Absolute path components[0] is always empty string

## Observed Behavior
For absolute paths, `path.components[0]` is always an empty string `""`, making it impossible to access the first directory with index 0.

```parsley
let p = @/home/hello
log(p.components)        // ["", "home", "hello"]
log(p.components[0])     // "" (empty!)
log(p.components[1])     // "home"
```

## Expected Behavior
Users expect `components[0]` to be the first meaningful path segment:

```parsley
let p = @/home/hello
log(p.components)        // ["home", "hello"] expected
log(p.components[0])     // "home" expected
```

## Reproduction Steps
1. Create an absolute path: `let p = @/home/hello`
2. Access components: `log(p.components)`
