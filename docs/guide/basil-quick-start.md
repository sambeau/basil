...
Your Parsley handlers receive request data from the `@basil/http` module:

**Accessing request data:**

```parsley
// `request` from @basil/http has this shape:
{
  method:     "GET",
  path:       "/api/hello",
  query:      {name: "world"},
...
**Custom Response** (set metadata via the `response` object):
...
| `<=??=>` | Query multiple rows | Table (use `.count()`, not `.length()`) |
