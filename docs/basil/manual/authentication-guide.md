...
For `fetch()` calls, send it as a header instead: `X-CSRF-Token`. Forget the token and you'll get a 403 — in dev mode, with an error page that tells you exactly which of the two tokens went missing.
