...
| #60 | Add `duration()` dynamic constructor | FEAT-087 | Consistency | Add `duration(string)` or `duration({days: 1, hours: 2})` builtin to dynamically create durations, matching pattern of other literal types. Currently no way to create duration from variable—must use literals like `@1d2h`. |
