...
- [ ] Survey how other frameworks handle this (Next.js `<Image>`, Eleventy image plugin, Hugo image processing) — what API patterns work well?
- [ ] Prototype two API shapes (width-descriptor vs density-descriptor) with real Parsley template examples and evaluate readability
- [ ] Determine whether a batch registry API is needed or if N×`Transform()` is acceptable for typical scale counts (3–5 variants)
- [ ] Decide on return type: string vs dict
...
- [ ] Decide: data URI vs URL (data URI is more useful but changes the function's return type contract)
- [ ] Test `disintegration/imaging` `Blur()` function: resize to 20px wide → blur σ=10 → encode JPEG q=20 → base64. Measure output size (should be <1KB)
- [ ] Prototype the API: which of the three approaches above reads best in real Parsley templates?
- [ ] Determine whether `imageInfo()` is the right home for this (returns it alongside metadata) or if it belongs on `image()`
...
- [ ] Evaluate pure-Go color extraction libraries (search for `dominant color`, `color palette`, `color quantization`)
- [ ] Test the "resize to 1×1" shortcut: `imaging.Resize(img, 1, 1, imaging.Lanczos)` → read pixel → format as hex. Compare quality against a simple average and a k-means approach on 5–10 diverse images
- [ ] Decide: always compute (adds full decode cost to `imageInfo()`) vs opt-in parameter
- [ ] Benchmark: how much time does full decode + resize-to-1×1 add vs header-only `DecodeConfig`?
...
- [ ] Evaluate `muesli/smartcrop`: check maintenance status, dependencies (must be pure Go, no CGo), star count, last commit, open issues
- [ ] Test `muesli/smartcrop` on 5–10 diverse images (portraits, landscapes, product shots, group photos): assess crop quality subjectively
- [ ] Benchmark: time to compute focal point on a typical 12MP JPEG
- [ ] Check if there are alternative pure-Go smart crop libraries
- [ ] Prototype integration: how does the focal point translate to an `imaging.Fill` anchor? Does `muesli/smartcrop` return coordinates that map to `imaging`'s anchor system, or do we need manual sub-image extraction?
...
- [ ] Test `imaging.Sharpen()` with sigma values 0.3, 0.5, 0.8 on 3–5 downscaled images. Compare visual quality (before/after, side by side)
- [ ] Decide: automatic vs opt-in
- [ ] Define the threshold and default sigma
- [ ] This is simple enough that it may not need a full design doc — could go straight to implementation after the above tests
...
**Investigation tasks:**
- [ ] Measure binary size impact of adding `gen2brain/webp` (build with and without, compare)
- [ ] Benchmark WebP encode performance via WASM on typical transforms
- [ ] Decide: include in Phase 2 or continue deferring?
...
**Investigation tasks:**
- [ ] Decide: add a simple `sync.Map` cache keyed on absolute path + modtime? Or defer until there's evidence of a real performance problem?
...
1. **Pick which items to include in Phase 2** — not all five need to ship together
2. **Run the investigation tasks** for selected items (marked with `[ ]` above)
3. **Write API proposals** for `imageSrcset()` and blur placeholders (the two with significant design decisions)
