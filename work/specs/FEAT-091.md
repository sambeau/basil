...
| `auto` | All | Boolean | Database/server generates this value |
| `readOnly` | All | Boolean | Field cannot be set from client/form input |
...
#### 3.3.1 The `auto` Constraint

The `auto` constraint marks fields whose values are generated by the database or server (e.g., auto-increment IDs, timestamps). This solves the "Catch-22" where a schema requires an `id` for completeness, but insert operations cannot provide one.
...
**SPEC-AUTO-001:** Fields with `auto` constraint MUST be skipped during insert validation.

**SPEC-AUTO-002:** Fields with `auto` constraint MUST be present and non-null in query results.

**SPEC-AUTO-003:** Fields with `auto` constraint MUST be immutable on update operations — attempting to change an `auto` field MUST produce an error.

**SPEC-AUTO-004:** `auto` and `required` MUST NOT be combined on the same field (they are contradictory for insert context).

**SPEC-AUTO-005:** `auto` and `default` MAY be combined — the default applies when the field is accessed before database generation.
...
| `@update` | Immutable (error if changed) |
| `@query` result | Always present |

**Typical use cases:**
- `id: id(auto)` — auto-increment or UUID primary keys
- `createdAt: datetime(auto)` — insertion timestamp
- `updatedAt: datetime(auto)` — modification timestamp

#### 3.3.2 The `readOnly` Constraint

The `readOnly` constraint marks fields that cannot be set from client/form input. This prevents privilege escalation attacks where users attempt to set sensitive fields like `role` via form submissions.
...
**SPEC-READONLY-001:** Fields with `readOnly` constraint MUST be silently filtered during record creation (`Schema({...})`).

**SPEC-READONLY-002:** Fields with `readOnly` constraint MUST be silently filtered during `record.update()`.

**SPEC-READONLY-003:** Fields with `readOnly` constraint MUST be readable via normal field access (`record.fieldName`).

**SPEC-READONLY-004:** The `readOnly` constraint is enforced at the schema level, not the database level. Different schemas MAY have different `readOnly` settings for the same underlying data.
...
| Forms/display | Visible (use `hidden` metadata to hide) |

**Multi-Schema Pattern:**

```parsley
// Public schema - role protected
@schema User {
    name: string
    role: enum["user", "admin"](readOnly, default: "user")
}

// Admin schema - role editable
@schema AdminUser {
    name: string
    role: enum["user", "admin"](default: "user")
}

// Same table, different security contexts
let Users = db.bind(User, "users")         // For public endpoints
let AdminUsers = db.bind(AdminUser, "users") // For admin endpoints
```
...
constraint      = "required" | "unique" | "auto"
                | "min" ":" (integer | string)
                | "max" ":" (integer | string)
                | "default" ":" literal ;
...
### C.1 `readOnly` Constraint

**Status:** ✅ Implemented (see section 3.3.2)

Fields that are:
- Readable by clients (displayed in UI, returned in API responses)
- Never writable from client/form input (filtered out during binding)
- Used to prevent privilege escalation (e.g., `role` field)
...
**Naming:** `readOnly` (matches OpenAPI semantics—server can write, client can only read)

**Enforcement level:** Schema-level (not database-level)
- The constraint affects binding behavior for *that schema*
- Different schemas can have different constraints on the same column
- This enables context-specific permission models

**Form binding:** Write protection only
- `readOnly` fields are filtered out when binding form/API data
- `readOnly` fields are still readable and displayable
- Use `hidden` metadata to control display: `role: ...(readOnly) | {hidden: true}`

**Interaction with `@update`:**
- `record.update(formData)` filters out `readOnly` fields
- To modify `readOnly` fields, use a different schema without the constraint

#### Multi-Schema Pattern

```parsley
// Public schema - role is protected
@schema User {
    id: id(auto)
    name: string
    role: enum["user", "admin"](readOnly, default: "user")
...
// Admin schema - role is editable  
@schema AdminUser {
    id: id(auto)
    name: string
    role: enum["user", "admin"](default: "user")  // No readOnly
}

// Same table, different bindings
let Users = db.bind(User, "users")
let AdminUsers = db.bind(AdminUser, "users")

// Public endpoint - role filtered out
post "/profile" {
    let user = Users.find(session.userId)
    user.update(req.form)  // Cannot change role
}

// Admin endpoint (behind auth middleware) - role editable
post "/admin/users/:id" {
    let user = AdminUsers.find(params.id)
    user.update(req.form)  // CAN change role
}
```

**Security model:**
1. Route-level auth controls access to admin endpoints
2. Schema binding controls what fields are writable
3. Never expose permissive schemas (e.g., `AdminUsers`) to public routes

#### Prior Art

| System | Equivalent | Notes |
|--------|------------|-------|
| OpenAPI | `readOnly` | Server→client only |
| GraphQL | Separate Input types | Type-level separation |
| Rails | strong parameters | Controller decides permitted fields |
| Django REST | `read_only_fields` | Serializer-level |
