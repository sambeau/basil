...
- **A durable place for site code to write** — `basil.data_dir`, `basil.uploads_dir` and `basil.uploads_url` are available to handlers, and `<data_dir>/uploads` is served at `/__uploads/`, following the existing `/__p/` and `/__img/` pattern. Uploads no longer have to live inside `public_dir`, where the next deploy would delete them. Directory listings are not served.
