...
### Fixed
- **Go toolchain bumped to 1.25.12** — the `go.mod` directive moved from 1.25.0 past the stdlib security fixes flagged in the pre-v1.0 audit (memory exhaustion in `net/url` query parsing, two TLS handshake issues, all fixed by 1.25.6/1.25.7). Builds with an older local toolchain fetch 1.25.12 automatically.
