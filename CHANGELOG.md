...
### Changed
- **Geomini display font for headings** — Headings (the homepage hero, section titles, and docs content headings) now use Geomini, a compact geometric sans (SIL Open Font License), inlined as a woff2 data URI so it always loads with no external request and no fallback flash. Body text stays on the system font. Single-weight face, so `font-synthesis: none` prevents a faked bold.
