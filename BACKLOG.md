...
| Item | Source | Reason Deferred | Notes |
|------|--------|-----------------|-------|
| `error()` function for user-defined catchable errors | FEAT-029 | Not MVP | Allow user functions to throw catchable errors: `error("message")` creates Value-class error caught by `try`. Enables user functions to participate in `try` error handling. Without this, `try` only catches builtin/system errors. |
