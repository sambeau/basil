...
| Item | Source | Reason Deferred | Notes |
|------|--------|-----------------|-------|
| `fail()` function for user-defined catchable errors | FEAT-029 | Not MVP | Allow user functions to throw catchable errors: `fail("message")` creates Value-class error caught by `try`. Enables user functions to participate in `try` error handling. Named `fail()` not `error()` to avoid namespace clash with `{result, error}` destructuring. See FEAT-030. |
